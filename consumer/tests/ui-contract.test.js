...
test('מסך הפתיחה אינו מציג תגית צפה ליד תמונת היועץ', async () => {
  ...
  assert.match(landing, /<span class="landing-tax-term"><i class="fas fa-circle-info"[^>]*><\/i>כדאי<\/span>/);
...
test('מסך התוצאה מציג יתרה פעם אחת, ספירת ימים רגועה ו-CTA לאחר ערך', async () => {
  const html = await readFile(new URL('../check.html', import.meta.url), 'utf8');
  const source = await readFile(new URL('../ui/app.js', import.meta.url), 'utf8');
  ...
  assert.match(html, /id="countdown-days"/);
  assert.doesNotMatch(html, /countdown-(?:seconds|minutes|hours)/);
  assert.doesNotMatch(source, /setInterval\(update, 1000\)/);

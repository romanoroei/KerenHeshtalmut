...
test('המחשבון המקצועי משתמש רק בשנת נתונים מאומתת ומציג הודעת fallback כללית', async () => {
  const html = await readFile(new URL('../../index.html', import.meta.url), 'utf8');
  assert.match(html, /function getVerifiedTaxContext\(requestedYear=CUR_YEAR\)/);
  assert.match(html, /status:'official', verifiedAt:'2026-07-19'/);
  assert.match(html, /נתוני \$\{requestedYear\} טרם אומתו\. החישוב מבוסס על הנתונים הרשמיים האחרונים שאומתו לשנת \$\{dataYear\}\./);
  assert.match(html, /const year\s+=ACTIVE_TAX_CONTEXT\.dataYear/);
  assert.match(html, /id="professionalTaxYearStatus" hidden/);
});
